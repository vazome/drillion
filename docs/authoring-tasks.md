...
`<NNN>` is the task's place in the curriculum, `001`–`313` with no gaps, so the next task you add is
`314`. It encodes no difficulty and no provenance, but it does encode order: a task's prereqs are
...
accepted, so if it still says `313/313` after you added one, `doctor` is where to look.
