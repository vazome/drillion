...
## You get
`tasks` — a list of job names like `["a", "b", "c"]`, never empty. The test creates it and hands it to you; you never build it yourself.

## You return
a pair `(buggy, fixed)`: the list the buggy function returns on its last call, and the list the safe version would return on its last call. You predict; you do not repair the function.
...
It is called once per item in `tasks`, always WITHOUT the second argument:
...
- `fixed` — what the last call would return if the default were done the safe way (`done=None`, make a fresh `[]` inside)

```python
solve(["a", "b", "c"])   # -> (["a", "b", "c"], ["c"])
```

`tasks` is never empty. Work it out on paper first — saying WHY the buggy one remembers earlier calls is the interview answer.
