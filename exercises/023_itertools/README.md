...
## Why
An on-call engineer is paged at 2am. The log service hands back log
...
## You get
`pages` — a list of lists of strings; each inner list is one
...
`first_n` — a whole number like 3: how many lines from the
start of the combined stream to look at.

## You return
a list of (service, count) pairs sorted by service name,
like [("api", 2), ("db", 1)].
