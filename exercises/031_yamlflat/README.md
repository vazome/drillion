...
## Why
Deployment settings live in a small text file: one setting per line
...
## You get
`text` — a multi-line string of settings, like the block shown
...
In real code this whole exercise is yaml.safe_load(text) — and never plain yaml.load, which can construct arbitrary Python objects from untrusted input.
