...
> [!NOTE]
> Topic 52 on the checklist is FastAPI. That needs a running server and packages we do not have offline, so this slot drills real-API pagination instead — the same loop as topic 49, but against actual requests/Response objects rather than a fake.
