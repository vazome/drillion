...
## Why
The deploy script has a step that posts an alert to the company
...
## You return
a dict with four keys: "result" (what run_deploy returned),
...
file. This module is `sys.modules[__name__]`, so
`patch.object(sys.modules[__name__], "send_alert")` is how you spell
that target — the dotted-string form `patch("pkg.send_alert")` cannot
name a module whose folder starts with a digit.

Constraint: no imports beyond unittest.mock and sys (`sys` is already
