...
## Why
This is the bug you fixed in the take-home, rebuilt small. The
...
## You get
nothing to start — you return an async function. The test then
...
## Read first
- https://realpython.com/async-io-python/  — 'Async IO Explained': an `await` is a place where this request pauses and lets OTHER requests run. Nothing else matters as much as that sentence.
- https://magicstack.github.io/asyncpg/current/usage.html#connection-pools  — the real pool you used: a fixed number of database connections, handed out one per `async with pool.acquire()`
- https://docs.python.org/3/library/asyncio-sync.html#asyncio.Semaphore  — the test's fake pool is just this: a counter of free slots
