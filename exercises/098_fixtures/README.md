...
## Why
The take-home README said "use pytest fixtures to set up and tear
...
## You get
`monkeypatch` — pytest hands this in because you named it as a
...
a function `run(rows, q)` (the "factory" pattern) that:
  ...
  2. patches `get_pool` in this module with an async fake returning a
     ...
     `fetch(vector)` returns `rows` (a FakePool from drill 096 is perfect),
...
- https://docs.pytest.org/en/stable/how-to/fixtures.html  — read 'What fixtures are', then 'Factories as fixtures' (a fixture that RETURNS A FUNCTION — exactly what you build here)
- https://docs.pytest.org/en/stable/how-to/monkeypatch.html  — setattr a fake in, and pytest puts the real thing back after the test, automatically
...
### Hint 3
SAY IT IN THE INTERVIEW: 'I should have put the two monkeypatch.setattr calls and the client construction in one fixture and taken it as a parameter in each test. Fixtures give one place for setup, automatic teardown — monkeypatch restores the originals after every test — and tests that only state the behaviour they check. Copy-pasting the setup six times is what the README's fixtures sentence was warning against.'
