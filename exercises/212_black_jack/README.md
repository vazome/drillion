...
> [!NOTE]
> Exercism asks for all six functions in one `black_jack.py`. Here the exercise is split in two: **this drill covers tasks 1–3**, and tasks 4–6 are drill `213_black_jack`. And there is one entry point: `solve()` takes **no arguments** and returns a dict that hands your three functions to the grader, keyed by name.
...
## Rules
- this drill implements **Exercism tasks 1, 2 and 3 only** — `is_blackjack`, `can_split_pairs` and `can_double_down` belong to drill `213_black_jack`
