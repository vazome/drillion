...
> [!NOTE]
> Exercism asks for all six functions in one `black_jack.py`. Here the exercise is split in two: tasks 1–3 are drill `212_black_jack`, and **this drill covers tasks 4–6**. And there is one entry point: `solve()` takes **no arguments** and returns a dict that hands your three functions to the grader, keyed by name.
