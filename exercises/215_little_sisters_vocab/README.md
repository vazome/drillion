...
## You get
Nothing. `solve()` takes **no arguments** — it hands the grader your four finished functions, and the grader calls them with the words and sentences. This drill covers all four Exercism tasks.
...
- [string methods](https://docs.python.org/3/library/stdtypes.html#string-methods) — the full menu; `join` and `split` are the two this drill lives on
