...
> [!NOTE]
> Exercism asks for all seven functions in one `list_methods.py`. Here the exercise is split in two: **this drill covers tasks 4–7**, and tasks 1–3 are drill `224_chaitanas_colossal_coaster`. There is one entry point — `solve()` returns a dict that hands your four functions to the grader, keyed by name.
...
## Rules
- this drill implements **Exercism tasks 4, 5, 6 and 7 only** — `add_me_to_the_queue`, `find_my_friend` and `add_me_with_my_friends` belong to drill `224_chaitanas_colossal_coaster`
