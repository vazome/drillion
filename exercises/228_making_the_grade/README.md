...
> [!NOTE]
> Exercism asks for all six functions in one `loops.py`. Here the exercise is split in two: **this drill covers tasks 4–6**, and tasks 1–3 are drill `227_making_the_grade`. There is one entry point — `solve()` returns a dict that hands your three functions to the grader, keyed by name.
...
## Rules
- this drill implements **Exercism tasks 4, 5 and 6 only** — `round_scores`, `count_failed_students` and `above_threshold` belong to drill `227_making_the_grade`
