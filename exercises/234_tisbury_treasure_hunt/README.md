...
> [!NOTE]
> Exercism asks for five functions in one `tuples.py`. Here the exercise is split in two: tasks 1–2 are drill `233_tisbury_treasure_hunt`, and **this drill covers tasks 3–5**. And there is one entry point: `solve()` takes **no arguments** and returns a dict that hands your three functions to the grader, keyed by name. Nothing stops you from writing your own coordinate-conversion helper and calling it from all three.
...
## Rules
- this drill implements **Exercism tasks 3, 4 and 5 only** — `get_coordinate` and `convert_coordinate` belong to drill `233_tisbury_treasure_hunt`
...
## Hints
### Hint 1
All three functions rest on the one small conversion from the previous drill: Azara's `'4B'` has to become `('4', 'B')` before it can be compared with anything of Rui's. Once it is in that shape, task 3 is a single [membership test](https://docs.python.org/3/reference/expressions.html#membership-test-operations) — Rui's record *is* a tuple, and `in` asks whether a value is one of its elements.
### Hint 2
