...
> [!NOTE]
> Exercism asks for five functions in one `dicts.py`. Here the exercise is split in two: tasks 1–3 are drill `236_inventory_management`, and **this drill covers tasks 4–5**. And there is one entry point: `solve()` takes **no arguments** and returns a dict that hands your two functions to the grader, keyed by name.
...
## Rules
- this drill implements **Exercism tasks 4 and 5 only** — `create_inventory`, `add_items` and `decrement_items` belong to drill `236_inventory_management`
