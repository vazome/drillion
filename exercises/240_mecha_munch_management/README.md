...
> [!NOTE]
> Exercism asks for six functions in one `dict_methods.py`. Here the exercise is split in two: tasks 1–3 are drill `239_mecha_munch_management`, and **this drill covers tasks 4–6**. And there is one entry point: `solve()` takes **no arguments** and returns a dict that hands your three functions to the grader, keyed by name.
...
## Rules
- this drill implements **Exercism tasks 4, 5 and 6 only** — `add_item`, `read_notes` and `update_recipes` belong to drill `239_mecha_munch_management`
