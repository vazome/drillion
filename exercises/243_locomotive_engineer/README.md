...
> [!NOTE]
> Exercism asks for five functions in one `locomotive_engineer.py`. Here the exercise is split in two: tasks 1–2 are drill `242_locomotive_engineer`, and **this drill covers tasks 3–5**. And there is one entry point: `solve()` takes **no arguments** and returns a dict that hands your three functions to the grader, keyed by name.
...
## Rules
- this drill implements **Exercism tasks 3, 4 and 5 only** — `get_list_of_wagons` and `fix_list_of_wagons` belong to drill `242_locomotive_engineer`
...
> [!NOTE]
> Exercism asks you to reach for packing, unpacking and multiple assignment here rather than for `dict` methods, slicing and index arithmetic. The tests cannot tell the difference — that is just what the exercise is for.
