...
## You get
The finished `Alien` class is already in the file above `solve`, marked `# given — do not edit`. It is exactly what drill `248_ellens_alien_game` asks you to write: `Alien(x_coordinate, y_coordinate)`, a `health` that starts at 3, and a class-level `total_aliens_created` that counts up on every construction.
...
> [!NOTE]
> Exercism calls this function `new_aliens_collection(positions)` and asks for it alongside the class in one `classes.py`. Here **this drill covers task 7 only** — the class itself is drill `248_ellens_alien_game` — and the entry point is `solve(positions)`.
