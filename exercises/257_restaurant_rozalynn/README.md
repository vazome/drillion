...
> [!NOTE]
> Exercism asks for all six functions in one `none.py`. Here the exercise is split in three: **this drill covers tasks 1 and 2** — building charts. Tasks 3 and 4 are drill `258_restaurant_rozalynn`; tasks 5 and 6 are drill `259_restaurant_rozalynn`.
...
> [!NOTE]
> Exercism's own test file for task 2 disagrees with its instructions here: the test expects the first guest to be dropped (`{1: 'Frank', ...}`), which is an off-by-one in Exercism's example solution. This drill grades the behaviour the **instructions** describe — `{1: 'Walter', 2: 'Frank', ...}`.
