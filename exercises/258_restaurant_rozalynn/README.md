...
minutes: 12
prereqs: [203, 209, 224, 227, 239, 257]
...
> [!NOTE]
> Exercism asks for all six functions in one `none.py`. Here the exercise is split in three: **this drill covers tasks 3 and 4** — reading a chart. Tasks 1 and 2 are drill `257_restaurant_rozalynn`; tasks 5 and 6 are drill `259_restaurant_rozalynn`.
...
> [!WARNING]
> Test with `if value is None`, not `if not value`. A truthiness test also fires for `0`, `""` and an empty list, so it would report a seat as empty for the wrong reason — and the graders for later drills in this set feed charts that punish it.
...
`timeout` is `0`, which is falsy but very much set — swap `value is None` for `not value` and it joins the missing list, which is the bug this exercise is teaching you to avoid.
