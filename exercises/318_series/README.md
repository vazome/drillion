---
title: series — every window of n digits, in order
minutes: 15
prereqs: [200, 206, 209, 215, 221, 227]
tags: [exercism, sequences, data-structures]
source: exercism/python practice/series (MIT, adapted)
---
# series — every window of n digits, in order

*series — the sliding window, plus four error messages that must be exact.*

## Why
A sliding window is the shape behind "the last five minutes of latency", "any three consecutive failed health checks", "every 4-byte frame in this payload". Getting the count of windows right — and it is `len - n + 1`, not `len // n` and not `len - n` — is the kind of off-by-one that ships. The other half of this drill is the guard clauses: four impossible inputs, four exact messages, checked before any work is done.
