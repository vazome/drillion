...
Guard, then a `while` on the stopping condition, then one counter incremented per iteration. The only thing this drill adds is a second branch inside the loop.
