...
> [!WARNING]
> Case is not folded here — unlike the Atbash drill, `"OMG"` must come back as `"TRL"`, in capitals. Shifting a capital with the lower-case alphabet's positions is the classic way to produce mojibake.
...
Position plus offset, then `%` the size of the ring so it comes back round to the beginning. In this drill the ring is the alphabet and the offset is the key.
