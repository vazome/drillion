"""The two ways in: serve the drills in a browser, or check the whole set still works.
    ...
    study             open the web UI
    study selfcheck   solve every exercise with its own _reference
...
def main(argv=None):
    ap = argparse.ArgumentParser(prog="study", description="Spaced-repetition Python drills.")
    ...
    if not settings.exercises_dir.is_dir():
        raise SystemExit(f"no exercises/ under {settings.root} — run study from the repo, "
                         f"or point STUDY_ROOT at the directory that holds it")
