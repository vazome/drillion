---
title: truthiness — defaults without swallowing 0 and ''
difficulty: medium
tier: core
minutes: 8
prereqs: []
tags: [truthiness]
---
# truthiness — defaults without swallowing 0 and ''
...
## You get
`port` — either `None` (not set), `0`, or a number like `8080`. `name` — either `None` (not set), `""` (empty text), or a word like `"api"`. The test creates them and hands them to you; you never build them yourself.

## You return
a pair `(port, name)` where `None` has been replaced with the default, and every other value, including `0` and `""`, comes back untouched.

## Rules
Apply defaults to two settings and return the tuple `(port, name)`.

- `port`: if it is `None`, use `8080`. But `0` is a REAL value (it means "let the OS pick a free port") and must come through untouched.
- `name`: if it is `None`, use `"worker"`. But `""` is a real value too.

```python
solve(None, "")   # -> (8080, "")
solve(0, None)    # -> (0, "worker")
```

> [!WARNING]
> The tempting one-liner `port or 8080` fails this test — the data includes `0` and `""` on purpose. One ternary per value does it.
