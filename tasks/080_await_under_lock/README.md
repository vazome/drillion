---
title: async pools — embed before you borrow
difficulty: medium
tier: advanced
minutes: 20
prereqs: [37, 53]
...
---
# async pools — embed before you borrow

*Do the slow wait BEFORE you borrow the connection.*
...
- [asyncpg connection pools](https://magicstack.github.io/asyncpg/current/usage.html#connection-pools) — the real thing: a fixed number of database connections, handed out one per `async with pool.acquire()`
...
## Why
A classic async performance bug, rebuilt small. A `/search` endpoint borrowed a database connection from a pool of 5, and THEN waited 80 ms for the embedding API while still holding it. Under 200 concurrent requests only 5 could be "inside" at a time, so the 80 ms waits ran five at a time instead of all at once: 40 rounds x 80 ms is over 3 seconds of pure queueing. The connection did nothing during that wait. Moving the embedding call before the borrow lets all 200 waits overlap, and the connection is held only for the few ms the query needs.
