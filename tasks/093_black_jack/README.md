...
> [!NOTE]
> Exercism asks for all six functions in one `black_jack.py`. Here the task is split in two: **this task covers tasks 1–3**, and tasks 4–6 are task `094_black_jack`. And there is one entry point: `solve()` takes **no arguments** and returns a dict that hands your three functions to the grader, keyed by name.
...
## Rules
- this task implements **Exercism tasks 1, 2 and 3 only** — `is_blackjack`, `can_split_pairs` and `can_double_down` belong to task `094_black_jack`
