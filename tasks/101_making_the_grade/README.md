...
> [!NOTE]
> Exercism asks for all six functions in one `loops.py`. Here the task is split in two: **this task covers tasks 1–3**, and tasks 4–6 are task `102_making_the_grade`. There is one entry point — `solve()` returns a dict that hands your three functions to the grader, keyed by name.
...
## Rules
- this task implements **Exercism tasks 1, 2 and 3 only** — `letter_grades`, `student_ranking` and `perfect_score` belong to task `102_making_the_grade`
