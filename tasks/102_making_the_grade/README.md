...
> [!NOTE]
> Exercism asks for all six functions in one `loops.py`. Here the task is split in two: **this task covers tasks 4–6**, and tasks 1–3 are task `101_making_the_grade`. There is one entry point — `solve()` returns a dict that hands your three functions to the grader, keyed by name.
...
## Rules
- this task implements **Exercism tasks 4, 5 and 6 only** — `round_scores`, `count_failed_students` and `above_threshold` belong to task `101_making_the_grade`
