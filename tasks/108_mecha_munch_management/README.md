...
> [!NOTE]
> Exercism asks for six functions in one `dict_methods.py`. Here the task is split in two: **this task covers tasks 1–3**, and tasks 4–6 are task `109_mecha_munch_management`. And there is one entry point: `solve()` takes **no arguments** and returns a dict that hands your three functions to the grader, keyed by name.
...
## Rules
- this task implements **Exercism tasks 1, 2 and 3 only** — `sort_entries`, `send_to_store` and `update_store_inventory` belong to task `109_mecha_munch_management`
