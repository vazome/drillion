...
2. patches `get_pool` in this module with an async fake returning a pool whose `acquire()` works in `async with` and whose conn's `fetch(vector)` returns `rows` (a FakePool from task 118 is perfect),
