...
> [!NOTE]
> Exercism asks for all six functions in one `black_jack.py`. Here the task is split in two: tasks 1–3 are task `212_black_jack`, and **this task covers tasks 4–6**. And there is one entry point: `solve()` takes **no arguments** and returns a dict that hands your three functions to the grader, keyed by name.
...
## Rules
- this task implements **Exercism tasks 4, 5 and 6 only** — `value_of_card`, `higher_card` and `value_of_ace` belong to task `212_black_jack`
