...
> [!NOTE]
> Exercism asks for all seven functions in one `lists.py`. Here the task is split in two: **this task covers tasks 1–3**, and tasks 4–7 are task `222_card_games`. There is one entry point — `solve()` returns a dict that hands your three functions to the grader, keyed by name.
...
## Rules
- this task implements **Exercism tasks 1, 2 and 3 only** — `card_average`, `approx_average_is_average`, `average_even_is_average_odd` and `maybe_double_last` belong to task `222_card_games`
