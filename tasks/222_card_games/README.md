...
> [!NOTE]
> Exercism asks for all seven functions in one `lists.py`. Here the task is split in two: **this task covers tasks 4–7**, and tasks 1–3 are task `221_card_games`. There is one entry point — `solve()` returns a dict that hands your four functions to the grader, keyed by name.
...
## Rules
- this task implements **Exercism tasks 4, 5, 6 and 7 only** — `get_rounds`, `concatenate_rounds` and `list_contains_round` belong to task `221_card_games`
