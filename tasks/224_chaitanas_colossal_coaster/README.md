...
> [!NOTE]
> Exercism asks for all seven functions in one `list_methods.py`. Here the task is split in two: **this task covers tasks 1–3**, and tasks 4–7 are task `225_chaitanas_colossal_coaster`. There is one entry point — `solve()` returns a dict that hands your three functions to the grader, keyed by name.
...
## Rules
- this task implements **Exercism tasks 1, 2 and 3 only** — `remove_the_mean_person`, `how_many_namefellows`, `remove_the_last_person` and `sorted_names` belong to task `225_chaitanas_colossal_coaster`
