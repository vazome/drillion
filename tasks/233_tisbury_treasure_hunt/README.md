...
> [!NOTE]
> Exercism asks for five functions in one `tuples.py`. Here the task is split in two: **this task covers tasks 1–2**, and tasks 3–5 are task `234_tisbury_treasure_hunt`. And there is one entry point: `solve()` takes **no arguments** and returns a dict that hands your two functions to the grader, keyed by name.
...
## Rules
- this task implements **Exercism tasks 1 and 2 only** — `compare_records`, `create_record` and `clean_up` belong to task `234_tisbury_treasure_hunt`
