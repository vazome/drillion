...
> [!NOTE]
> Exercism asks for five functions in one `locomotive_engineer.py`. Here the task is split in two: **this task covers tasks 1–2**, and tasks 3–5 are task `243_locomotive_engineer`. And there is one entry point: `solve()` takes **no arguments** and returns a dict that hands your two functions to the grader, keyed by name.
...
## Rules
- this task implements **Exercism tasks 1 and 2 only** — `add_missing_stops`, `extend_route_information` and `fix_wagon_depot` belong to task `243_locomotive_engineer`
