...
> [!NOTE]
> Exercism asks for five functions in one `locomotive_engineer.py`. Here the task is split in two: tasks 1–2 are task `242_locomotive_engineer`, and **this task covers tasks 3–5**. And there is one entry point: `solve()` takes **no arguments** and returns a dict that hands your three functions to the grader, keyed by name.
...
## Rules
- this task implements **Exercism tasks 3, 4 and 5 only** — `get_list_of_wagons` and `fix_list_of_wagons` belong to task `242_locomotive_engineer`
