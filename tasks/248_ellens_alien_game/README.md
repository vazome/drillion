...
> [!NOTE]
> Exercism asks for the class plus a standalone `new_aliens_collection()` function in one `classes.py`. Here the task is split in two: **this task covers tasks 1–6**, the class itself. Task 7, the function that builds a list of aliens from a list of positions, is task `249_ellens_alien_game`.
...
## Rules
- this task implements **Exercism tasks 1 to 6 only** — `new_aliens_collection` belongs to task `249_ellens_alien_game`
