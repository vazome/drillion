"""What one sitting of 274 asks for, and what counts as having answered it.
...
def check(doc, b):
    ...
    spec = doc.get("spec", {})
    assert spec.get("type") == "LoadBalancer", (
        f"spec.type is {spec.get('type')!r}, and the task asks for a LoadBalancer"
    )
    ...
    assert node_port is not None, (
        "spec.ports[0].nodePort is missing: the balancer's back end is the nodes' "
        "NodePorts, so the hole in the firewall is still part of the shape"
    )
