...
gunicorn listens on `127.0.0.1:8000` unless told otherwise. Inside a container that address is the container itself, unreachable from outside it, so a server in a container binds `0.0.0.0`.

## You get
A build context with a Flask app and its `requirements.txt`, open in the tabs above the editor.

## You return
A Dockerfile that installs the requirements, then copies the app, on Python `{python}`, served by gunicorn on port `{port}`.
...
- `EXPOSE {port}`
- one `CMD` in exec form: `gunicorn` bound to `0.0.0.0:{port}`, serving `app:app`
...
### Hint 3
gunicorn takes its options before the app, and every option and value is its own string in the array:

```dockerfile
CMD ["gunicorn", "--bind", "0.0.0.0:7000", "shop:api"]
```
