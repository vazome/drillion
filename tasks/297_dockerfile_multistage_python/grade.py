...
def check(stages, b):
    ...
    made = [s for s in build.all("RUN") if "venv" in s["words"]]
    ...
    installs = [s for s in build.all("RUN") if "install" in s["words"] and any(w.endswith("pip") for w in s["words"])]
    ...
    assert not [s for s in final.all("RUN") if "install" in s["words"]], (
