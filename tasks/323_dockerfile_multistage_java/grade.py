...
def _mvn(stage, goal):
    return [s for s in stage.all("RUN") if s["words"][:1] == ["mvn"] and goal in s["words"]]
...
def check(stages, b):
    ...
    workdir = [w for s in build.all("WORKDIR") for w in s["words"]]
    ...
    assert not final.all("RUN"), "the runtime stage builds nothing: it only receives the jar"
    copied = [c for c in final.all("COPY") if c["flags"].get("from") == build["name"]]
    ...
    assert not [c for c in final.all("COPY") if not c["flags"].get("from")], (
        "the runtime stage copies nothing from the context: the jar holds the app"
    )
    workdir = [w for s in final.all("WORKDIR") for w in s["words"]]
    ...
    env = [w for s in final.all("ENV") for w in s["words"]]
