...
tasks/017_fstrings/task.py:24:
_ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _

>       raise NotImplementedError
E       NotImplementedError

tasks/017_fstrings/task.py:2: NotImplementedError
=========================== short test summary info ============================
FAILED tasks/017_fstrings/task.py::test_solve - NotImplementedError
"""
...
def test_summarise_maps_the_region_to_editor_lines():
    ...
    assert "tasks/017_fstrings/task.py:24:" in out["output"]  # the test frame stays put


def test_summarise_falls_back_to_the_failed_line_and_caps_the_headline():
    only_failed = "FAILED tasks/017_fstrings/task.py::test_solve - boom\n"
...
def test_a_failure_names_the_task_the_short_way(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(settings, "root", tmp_path)
    task = tmp_path / "tasks" / "017_fstrings" / "task.py"
    task.parent.mkdir(parents=True)
    task.write_text("def test_solve():\n    assert 1 == 2\n", encoding="utf-8")
    _, out = runner.run_tests(task, seed=1)
    text = runner.summarise(out, marker_line=1)["output"]
    assert "../tasks/017_fstrings/task.py" in text
    assert "task.py" not in text.replace("../tasks/017_fstrings/task.py", ""), text
...
def test_selfcheck_solves_each_task_with_its_own_reference(
    ...
    for slug in ("017_fstrings", "018_slicing"):
    ...
    assert "FAILED 017_fstrings" in capsys.readouterr().out


WINDOWS_OUT = (
    "..\\tasks\\017_fstrings\\task.py:9: AssertionError\n"
    "=========================== short test summary info ===========================\n"
    "FAILED ..\\tasks\\017_fstrings\\task.py::test_solve - assert 1 == 2\n"
)
...
def test_a_windows_path_reaches_the_panel_the_same_way_a_posix_one_does():
    ...
    assert "../tasks/017_fstrings/task.py:9" in text
    assert "\\" not in text
...
def test_a_failed_line_names_the_task_folder_and_nothing_else(sep):
    ...
    line = f"FAILED ..{sep}tasks{sep}017_fstrings{sep}_selfcheck.py::test_solve - boom"
    collected = f"ERROR ..{sep}tasks{sep}018_slicing{sep}_selfcheck.py"
