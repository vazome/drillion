...
def test_a_fresh_clone_starts_with_an_empty_ladder():
    ...
    def check(_tmp):
        ...
        assert state.load() == {"focus": None, "cards": {}, "open": {}, "log": [],
                                "archive": {}}
        assert not settings.state_path.exists()
    _root(check)
...
def test_an_existing_progress_file_upgrades_untouched():
    ...
    stored = {"focus": "class-inheritance",
              "cards": {"008_sortkey": {"box": 3, "due": "2026-09-01", "seen": 4}},
              "open": {"021_sets": {"attempts": 2, "hints": 1, "active": 640, "seed": 7,
                                    "last": "2026-08-26T10:00:00", "solution_shown": False}},
              "log": [{"date": "2026-08-20", "slug": "008_sortkey", "grade": "pass",
                       "attempts": 1, "secs": 300, "new": True}],
              "archive": {"008_sortkey": [{"date": "2026-08-20", "grade": "pass",
...
def test_the_repo_does_not_ship_anybody_s_progress():
    ...
    tracked = subprocess.run(["git", "ls-files", "progress.json", "progress.json.bak"],
