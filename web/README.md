...
pnpm --dir web screens          # Playwright: renders all 323 task pages, photographs the rest,
...
- `src/ds/` is the drillion design system ("Mineral Blue"), authored in Claude Design and
  ...
  `Dialog.jsx` (the Settings modal) keeps local transitions and close handling that the
  upstream one lacks; NOTES.md lists them.
- `e2e/a11y.spec.ts` is the accessibility gate: a keyboard-only pass over the loop, the focus
  ...
  does not churn three sets of captures. Every WCAG A and AA rule runs, `color-contrast`
