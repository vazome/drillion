  ...
  return (
    <div style={{ display: "grid", gap: "10px", ...style }}>
      ...
      <textarea value={value} rows={rows} placeholder={placeholder} aria-label={ariaLabel || label}
        ...
        style={{ width: "100%", boxSizing: "border-box", display: "block", resize: "vertical", fontFamily: "var(--font-sans)", fontSize: "14px", lineHeight: "1.55", padding: "8px 12px", borderRadius: "var(--radius)", border: "1px solid var(--border-strong)", background: "var(--surface-2)", color: "var(--text)", outline: "none", transition: "border-color var(--dur-press) var(--ease-out)" }}></textarea>
    </div>
  );
}
