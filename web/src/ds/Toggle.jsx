...
export function Toggle({ checked = false, onChange, label, disabled = false, ariaLabel, style }) {
  ...
  return (
    <button type="button" role="switch" aria-checked={!!checked} aria-label={label ? undefined : ariaLabel} disabled={disabled}
      ...
      <span aria-hidden="true" style={{ position: "relative", width: "34px", height: "20px", flex: "none", borderRadius: "var(--radius-pill)", boxSizing: "border-box", border: "1px solid " + (on ? "var(--accent)" : "var(--border-strong)"), background: disabled ? "var(--surface-2)" : (on ? (hover ? "var(--accent-hover)" : "var(--accent)") : (hover ? "var(--surface-2)" : "var(--surface)")), transition: "background .12s, border-color .12s" }}>
        <span style={{ position: "absolute", top: "2px", left: checked ? "16px" : "2px", width: "14px", height: "14px", borderRadius: "var(--radius-pill)", background: disabled ? "var(--text-faint)" : (on ? "var(--on-accent)" : "var(--border-strong)"), transition: "left .15s ease-out" }}></span>
      </span>
